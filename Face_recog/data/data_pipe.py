...
def get_train_loader(conf):
    ...
    if conf.data_mode == 'vgg':
        ...
        ds, class_num = get_train_dataset('D:\Dataset\Face\imgs') #
    loader = DataLoader(ds, batch_size=conf.batch_size, shuffle=True, pin_memory=conf.pin_memory, num_workers=conf.num_workers)
    return loader, class_num


def get_train_loader_VTT(conf):

    # ds, class_num = get_train_dataset('D:\Dataset\Face\imgs') # Set you data path
    ds, class_num = get_train_dataset('D:\PROPOSAL\VTT\data\AnotherMissOh\Face\imgs') # Set you data path
    loader = DataLoader(ds, batch_size=conf.batch_size, shuffle=True, pin_memory=conf.pin_memory, num_workers=conf.num_workers)
...
def get_val_data(data_path):
    agedb_30, agedb_30_issame = get_val_pair(Path(data_path), 'agedb_30')
    cfp_fp, cfp_fp_issame = get_val_pair(Path(data_path), 'cfp_fp')
    lfw, lfw_issame = get_val_pair(Path(data_path), 'lfw')
